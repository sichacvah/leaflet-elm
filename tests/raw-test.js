...
Elm.Leaflet.Geometry = Elm.Leaflet.Geometry || {};
Elm.Leaflet.Geometry.Models = Elm.Leaflet.Geometry.Models || {};
Elm.Leaflet.Geometry.Models.make = function (_elm) {
   "use strict";
   _elm.Leaflet = _elm.Leaflet || {};
   _elm.Leaflet.Geometry = _elm.Leaflet.Geometry || {};
   _elm.Leaflet.Geometry.Models = _elm.Leaflet.Geometry.Models || {};
   if (_elm.Leaflet.Geometry.Models.values) return _elm.Leaflet.Geometry.Models.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var Bounds = F2(function (a,b) {    return {min: a,max: b};});
   var Point = F2(function (a,b) {    return {x: a,y: b};});
   var initBounds = F2(function (point1,point2) {
      var maxY = A2($Basics.max,point1.y,point2.y);
      var minY = A2($Basics.min,point1.y,point2.y);
      var maxX = A2($Basics.max,point1.x,point2.x);
      var minX = A2($Basics.min,point1.x,point2.x);
      return A2(Bounds,A2(Point,minX,minY),A2(Point,maxX,maxY));
   });
   var initPoint = F3(function (needRound,x,y) {
      return needRound ? A2(Point,$Basics.toFloat($Basics.round(x)),$Basics.toFloat($Basics.round(y))) : A2(Point,x,y);
   });
   var Transformation = F2(function (a,b) {    return {transform: a,untransform: b};});
   return _elm.Leaflet.Geometry.Models.values = {_op: _op
...
Elm.Leaflet = Elm.Leaflet || {};
Elm.Leaflet.Geometry = Elm.Leaflet.Geometry || {};
Elm.Leaflet.Geometry.Point = Elm.Leaflet.Geometry.Point || {};
Elm.Leaflet.Geometry.Point.make = function (_elm) {
   "use strict";
   _elm.Leaflet = _elm.Leaflet || {};
   _elm.Leaflet.Geometry = _elm.Leaflet.Geometry || {};
   _elm.Leaflet.Geometry.Point = _elm.Leaflet.Geometry.Point || {};
   if (_elm.Leaflet.Geometry.Point.values) return _elm.Leaflet.Geometry.Point.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Leaflet$Geometry$Models = Elm.Leaflet.Geometry.Models.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var quals = F2(function (point1,point2) {    return _U.eq(point1.x,point2.x) && _U.eq(point1.y,point2.y);});
   var distanceTo = F2(function (point1,point2) {
      var y = point1.y - point2.y;
      var x = point1.x - point2.x;
      return $Basics.sqrt(Math.pow(x,2) + Math.pow(y,2));
   });
   var ceilPoint = function (point) {    return _U.update(point,{x: $Basics.toFloat($Basics.ceiling(point.x)),y: $Basics.toFloat($Basics.ceiling(point.y))});};
   var floorPoint = function (point) {    return _U.update(point,{x: $Basics.toFloat($Basics.floor(point.x)),y: $Basics.toFloat($Basics.floor(point.y))});};
   var roundPoint = function (point) {    return _U.update(point,{x: $Basics.toFloat($Basics.round(point.x)),y: $Basics.toFloat($Basics.round(point.y))});};
   var unscaleBy = F2(function (point1,point2) {    return _U.update(point1,{x: point1.x * point2.x,y: point1.y * point2.y});});
   var scaleBy = F2(function (point1,point2) {    return _U.update(point1,{x: point1.x * point2.x,y: point1.y * point2.y});});
   var divideBy = F2(function (point,num) {    return _U.update(point,{x: point.x / num,y: point.y / num});});
   var substract = F2(function (point1,point2) {    return _U.update(point1,{x: point1.x - point2.x,y: point1.y - point2.y});});
   var add = F2(function (point1,point2) {    return _U.update(point1,{x: point1.x + point2.x,y: point2.y + point2.y});});
   return _elm.Leaflet.Geometry.Point.values = {_op: _op
                                               ,add: add
                                               ,substract: substract
                                               ,divideBy: divideBy
                                               ,scaleBy: scaleBy
...
Elm.Tests.make = function (_elm) {
   ...
   $Leaflet$Geometry$Models = Elm.Leaflet.Geometry.Models.make(_elm),
   $Leaflet$Geometry$Point = Elm.Leaflet.Geometry.Point.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm);
   var _op = {};
   var resultPoint = A2($Leaflet$Geometry$Models.Point,10.5 + 2.2,11.5 + 3.5);
   var secondPoint = A2($Leaflet$Geometry$Models.Point,2.2,3.5);
   var firstPoint = A2($Leaflet$Geometry$Models.Point,10.5,11.5);
   var all = A2($ElmTest.suite,
   "A Test Suite",
   _U.list([A2($ElmTest.test,"Addition",A2($ElmTest.assertEqual,A2($Leaflet$Geometry$Point.add,firstPoint,secondPoint),resultPoint))
   ...
   return _elm.Tests.values = {_op: _op,firstPoint: firstPoint,secondPoint: secondPoint,resultPoint: resultPoint,all: all};
};
Elm.Main = Elm.Main || {};
